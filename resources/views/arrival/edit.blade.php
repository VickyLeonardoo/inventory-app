                ...
                <form method="POST" action="{{ route('arrival.update', $arrival) }}" enctype="multipart/form-data">
                        ...
                        <select id="supplier_id" name="supplier_id"
                            ...
                            @foreach ($suppliers as $supplier)
                                <option value="{{ $supplier->id }}"
                                    {{ $supplier->id === $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}
                                </option>
                            @endforeach
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="quantity" class="block mt-1 w-full" type="number" name="quantity"
                            value="{{ $arrival->quantity }}" required autofocus autocomplete="quantity" />
                        <x-input-error :messages="$errors->get('quantity')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="arrival_date" :value="__('Arrival Date')" />
                        <x-text-input id="arrival_date" class="block mt-1 w-full" type="date" name="arrival_date"
                            value="{{ $arrival->arrival_date }}" required autofocus autocomplete="arrival_date" />
