<x-app-layout>
 ...
 <!-- Name -->
                <form method="POST" action="{{ route('application.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="mt-4">
                        <x-input-label for="name" :value="__('Issued Name')" />
                        <x-text-input id="name" class="block mt-1 w-full bg-gray-100 text-gray-500 cursor-not-allowed opacity-70" type="text" name="name" value="{{ auth()->user()->name }}" disabled readonly autofocus autocomplete="name" />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>
                    
                    <div class="mt-4">
                        <x-input-label for="application_date" :value="__('Application Date')" />
                        <x-text-input id="application_date" class="block mt-1 w-full" type="date" name="application_date" :value="old('application_date')" required autofocus autocomplete="application_date" />
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateAvailableItems() {
                ...
                allSelects.forEach(select => {
                    ...
                    if (currentValue && !Array.from(select.options).some(option => option.value === currentValue && option.style.display !== 'none')) {
                        select.value = '';
                    }
                });
            }
